...
public class Duelo {
	...
	public Personaje combatir() {
		...
		System.out.println("Bienvenidos a la pelea!!");
		...
		do {
			...
			if(pFinal.getPuntosVida() <= 0) {
				combateAcabado = true;
				pGanador = pInicial;
				System.out.println("Combatiente " + pFinal.getNombre() + " a muerto");
			}else {
				pFinal.atacar(pInicial);
				if(pInicial.getPuntosVida() <= 0) {
					pGanador = pFinal;
					combateAcabado = true;
					System.out.println("Combatiente " + pInicial.getNombre() + " a muerto");
				}
			}
